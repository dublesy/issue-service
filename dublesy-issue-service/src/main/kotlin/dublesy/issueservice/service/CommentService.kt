...
import dublesy.issueservice.domain.CommentRepository
import dublesy.issueservice.domain.IssueRepository
import dublesy.issueservice.exception.NotFoundException
import dublesy.issueservice.model.CommentRequest
import dublesy.issueservice.model.CommentResponse
import dublesy.issueservice.model.toResponse
...
class CommentService(
    ...
    fun delete(issueId: Long, id: Long, userId: Long) {
        val issue = issueRepository.findByIdOrNull(issueId) ?: throw NotFoundException("이슈가 존재하지 않습니다.")
        commentRepository.findByIdAndUserId(id, userId)?.let { comment -> issue.comments.remove(comment) }
    }
}